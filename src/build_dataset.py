...
_MS  = r"(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)"
_M   = r"(?:" + _ML + r"|" + _MS + r")\.?"
...
def sent_split(text):
    """Split contract clause text into sentences using nltk (Punkt tokenizer).

    Punkt handles abbreviations (e.g. "Inc.", "Dec.") and decimal numbers
    correctly, avoiding the false splits caused by the previous regex approach.
    Long sentences (>500 chars) are further split on semicolons.
    Short fragments (<8 tokens) are merged with the preceding sentence.
    """
...
def bio_tag_regex(tokens, ctype, anchor_iso=None):
    ...
    NOTE — SILVER LABELS: Tags produced here are automatically generated by
    regex pattern matching, NOT human-annotated gold labels.  Expect ~5-15%
    noise (missed spans, wrong span boundaries, spurious year-only matches).
    Downstream NER models should be evaluated against held-out human-labelled
    data before being used in production.  MLflow runs set tag
    label_source='silver_regex' to make this provenance explicit.
    """
    if ctype == "none":
        return ["O"] * len(tokens)
    ...
    if ctype in ("expiration", "effective", "agreement"):
        if ctype == "expiration":
            ...
            # NOTE: agreement dates share the START_DATE entity type with effective dates.
            # The NER model cannot distinguish them at the entity level; disambiguation
            # relies entirely on the upstream classifier label (agreement vs effective).
            # Future improvement: introduce a dedicated B/I-AGREEMENT_DATE entity type.
            b_tag, i_tag = "B-START_DATE", "I-START_DATE"
